import { AlertTriangle, Clock, ArrowRight, Ambulance } from "lucide-react";
...
const getTriageStyles = (triage: string) => {
  switch (triage) {
    case "Red": return { badge: "bg-destructive text-destructive-foreground", ring: "ring-destructive/30", pulse: true };
    case "Orange": return { badge: "bg-warning text-warning-foreground", ring: "ring-warning/30", pulse: false };
    case "Yellow": return { badge: "bg-yellow-500 text-white", ring: "ring-yellow-500/30", pulse: false };
    case "Green": return { badge: "bg-success text-success-foreground", ring: "ring-success/30", pulse: false };
  ...
  return (
    <div className="animate-fade-in rounded-xl bg-card shadow-card" style={{ animationDelay: "600ms" }}>
      ...
      <div className="divide-y divide-border/50">
        {emergencyPatients.map((patient: any, index: number) => {
      ...
      <div className="border-t border-border p-3">
        <Button variant="ghost" className="w-full justify-center text-primary">View Full Queue</Button>
      </div>
    </div>
  );
};
